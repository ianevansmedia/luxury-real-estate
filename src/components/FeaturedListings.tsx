...
export default function FeaturedListings() {
  // 1. State to hold the houses from the database
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      const { data, error } = await supabase
        ...
        .select('*')
        .limit(6) // Fetch top 6
        .order('id', { ascending: true }); // or order by 'created_at'
  ...
  return (
    <div className="max-w-[1600px] mx-auto px-6 md:px-12 py-24">
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-20">
          {properties.map((p) => (
             // 3. Wrap in Link to make it clickable
             <Link href={`/property/${p.id}`} key={p.id} className="group cursor-pointer">
                ...
                <div className="relative h-[450px] mb-6 overflow-hidden border border-white/10 p-1 group-hover:border-[#D4AF37]/50 transition-colors duration-500">
                   <div className="relative w-full h-full overflow-hidden bg-[#111]">
                      {/* Note: Using 'image_url' because that is the column name in Supabase */}
                      <Image 
                         src={p.image_url} 
                         alt={p.title} 
                         fill 
